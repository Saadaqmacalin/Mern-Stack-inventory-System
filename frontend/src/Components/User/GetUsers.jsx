...
const GetUsers = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto mt-10 px-4">
      ...
      />
<span className="ml-30 h-35 w-35 py-2 px-2 text-white bg-indigo-500 text-2xl font-semibold rounded-2xl">Total Users:{users.length}</span>
      <button
          ...
          <thead className="bg-indigo-500 text-white">
            <tr>
              {[ "#","Name", "Email", "Status", "Actions"].map((head) => (
                <th key={head} className="px-6 py-3 text-left text-sm font-medium uppercase tracking-wider">
                  {head}
                </th>
              ))}
            </tr>
              ...
              filteredData.map((u,index) => (
                <tr key={u._id} className="border-b hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-3 flex gap-2">
                    ...
                    <button onClick={() => handleEdit(u)} className="bg-blue-500 text-white px-3 py-1 rounded-lg text-sm font-medium hover:bg-blue-700 transition">
                      Edit
                    </button>
                    <button onClick={() => handleDelete(u._id)} className="bg-red-500 text-white px-3 py-1 rounded-lg text-sm font-medium hover:bg-red-700 transition">
                      Delete
                    </button>
                  </td>
                </tr>
